...
function activate(context) {
    ...
    // Регистрируем команду с новым именем
    let disposable = vscode.commands.registerCommand('chibi-motivator.showChibi', () => {
        showChibi(context);
    });
    context.subscriptions.push(disposable);
    // Автоматически показываем чибика при запуске (опционально)
    // showChibi(context);
}
exports.activate = activate;
function showChibi(context) {
    if (chibiPanel) {
        chibiPanel.reveal();
        return;
    }
    // Создаём панель для чибика
    chibiPanel = vscode.window.createWebviewPanel('chibiPanel', 'Chibi Companion 🎀', vscode.ViewColumn.Beside, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
    // Устанавливаем HTML с анимированным чибиком
